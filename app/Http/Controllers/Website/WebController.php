...
class WebController extends Controller
{
  ...
  public function webIndex()
  {
    ...
    if ($config[43]->config_value == "yes") {
      ...
      /*---============== SANTHOSH =============--->*/
      // Business URLs
      $business_array = Business::where('status', 1)
        ->select('business_name', 'business_website_url')
        ->get()
        ->toArray();
  ...
  public function business($param = null)
  {
    ...
    if (!$param) {
      return response()->json(['error' => 'No valid parameter provided'], 400);
    }
    /*---============== SANTHOSH =============---*/
    ...
    if ($config[43]->config_value == "yes") {
      ...
      if (filter_var($param, FILTER_VALIDATE_URL)) {
        ...
        // Get business id
        $business_id = Business::where('business_website_url', 'like', "%{$param}%")->first()?->business_id;
        $business_website_url = $param;
      } else {
        // Get business user id
        $business_user_id = Business::where('business_id', $param)->first()->user_id;
        $business_id = $param;
      }
